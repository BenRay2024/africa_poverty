...
def tfexporter(collection: ee.FeatureCollection, export: str, prefix: str,
    ...
    if dropselectors is not None:
        if selectors is None:
            selectors = ee.Feature(collection.first()).propertyNames()
...
def sample_patch(point: ee.Feature, patches_array: ee.Image,
    ...
    arrays_samples = patches_array.sample(
        ...
        dropNulls=False,
        tileScale=12)
    return arrays_samples.copyProperties(point, point.propertyNames())
...
def get_array_patches(
    ...
    samples = points.map(lambda pt: sample_patch(pt, patches_array, scale)).flatten()
